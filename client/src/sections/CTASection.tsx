...
export default function CTASection() {
    ...
    return (
        ...
        >
            ...
            <motion.button onClick={() => navigate('/generate')} className="px-12 py-3 text-slate-800 bg-white hover:bg-slate-200 rounded-full text-sm mt-4 font-semibold cursor-pointer"
                initial={{ y: 80, opacity: 0 }}
                whileInView={{ y: 0, opacity: 1 }}
                viewport={{ once: true }}
                transition={{ type: "spring", stiffness: 280, damping: 70, mass: 1 }}
            >
                Generate Free Content Pack
            </motion.button>
        </motion.div>
    );
}