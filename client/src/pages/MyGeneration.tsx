...
const MyGeneration = () => {
  ...
  const [loading , setLoading] = useState(false); //keep it false
          ...
          {loading && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from( {length : 6}).map( (_ , i) => (
                <div key={i} className="rounded-2xl bg-white/6 border border-white/10 animate animate-pulse h-[260px]"/>
              ))}
            </div>
          )}
                  ...
                  className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                    ...
                    <div className="p-4 space-y-2">
                        <h3 className="text-sm font-semibold text-zinc-100 line-clamp-2">{thumb.title}</h3>
                          ...
                          <p className="text-zinc-500 text-xs">{new Date(thumb.createdAt!).toDateString()}</p>
                    </div>
